...
def admin_menu():
    ...
    kb.add(types.InlineKeyboardButton("📩 إذاعة رسالة", callback_data="broadcast"))
    kb.add(types.InlineKeyboardButton("🔁 إذاعة توجيه", callback_data="forward"))
...
def check_sub(c):
    if subscribed(c.from_user.id):
        bot.edit_message_text("✅ تم التحقق\nاكتب /start", c.message.chat.id, c.message.message_id)
    else:
...
def collect(m):
    ...
    if now - u["last_collect"] < 86400:
        ...
        bot.send_message(m.chat.id, f"⏳ حاول بعد {left//3600} ساعة")
        return
    u["points"] += 5
    u["last_collect"] = now
...
def adm(c):
    ...
    if c.data == "charge":
        ...
        bot.send_message(c.message.chat.id, "📩 اكتب رسالة الإذاعة")
    elif c.data == "forward":
        bot.send_message(c.message.chat.id, "🔁 ابعت الرسالة للتوجيه")
    elif c.data == "stats":
        s = data["stats"]
    ...
    if s == "charge":
        ...
        for uid in data["users"]:
            try:
                ...
                pass
        bot.send_message(m.chat.id, "✅ تم التوجيه")
